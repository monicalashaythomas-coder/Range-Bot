...
CONFIG = {
    ...
    # Stability gate: mc_p_win_expiryrange uses genuine random sampling
    # (Box-Muller draws per simulation), so a single run can look
    # confident purely by chance. Running it mc_runs independent times
    # and requiring BOTH mean >= mc_min_confidence AND std <= mc_max_std
    # guards against exactly that — a signal only fires if it's
    # consistently confident across independent batches of draws, not
    # just once. Only applies to expiryrange: touch/notouch use a
    # closed-form analytic formula (p_touch_upper/p_touch_lower) with no
    # randomness at all, so re-running them would always return the
    # identical number — a stability gate there would be a no-op that
    # trivially "passes" every time, not a real check.
    "mc_runs":            _env("MC_RUNS", 5),
    "mc_max_std":         _env("MC_MAX_STD", 0.04),
...
def mc_multi_eval(mc_fn, ts: TickStore, barrier: float, duration_s: int,
                   runs: int, n_sims: int) -> Tuple[float, float, List[float]]:
    """
    Runs mc_fn independently `runs` times and returns (mean, std, all_runs).

    Ported from a design review of a separate bot draft — genuinely a
    better MC gate than a single point estimate, since mc_p_win_expiryrange
    draws from real randomness (Box-Muller per simulation): a single run
    of n_sims draws can land on a confident-looking p_win purely by
    sampling luck. Requiring the mean across several independent runs to
    clear the threshold AND the spread across those runs to stay low
    (see mc_max_std) means a fired signal reflects a genuinely stable
    estimate, not one lucky batch.
    """
    results = [mc_fn(ts, barrier, duration_s, n_sims) for _ in range(runs)]
...
def evaluate_expiryrange_signal(ts: TickStore, cfg: dict) -> ContractSignal:
    ...
    # Multi-run stability gate — see mc_multi_eval docstring for why this
    # matters more than a single point estimate.
    mean_p, std_p, all_runs = mc_multi_eval(
        mc_p_win_expiryrange, ts, barrier, dur,
        runs=cfg["mc_runs"], n_sims=cfg["mc_n_sims"])
    run_str = " ".join(f"{r:.3f}" for r in all_runs)
    reasons.append(f"MC mean={mean_p:.3f} std={std_p:.3f} runs=[{run_str}] "
                   f"(min_mean={cfg['mc_min_confidence']:.2f} "
                   f"max_std={cfg['mc_max_std']:.3f})")

    if mean_p < cfg["mc_min_confidence"]:
        return ContractSignal("SKIP", sym, barrier, mean_p, layer_score, rq_score,
                              reasons + ["MC mean below confidence threshold"])

    if std_p > cfg["mc_max_std"]:
        return ContractSignal("SKIP", sym, barrier, mean_p, layer_score, rq_score,
                              reasons + ["MC unstable across runs (std too high)"])

    return ContractSignal("EXPIRYRANGE", sym, barrier, mean_p,
...
def evaluate_touch_signal(ts: TickStore, cfg: dict) -> ContractSignal:
    ...
    NOTE ON THE MC STABILITY GATE (mc_runs/mc_max_std, used by
    evaluate_expiryrange_signal): deliberately NOT applied here.
    p_touch_upper/p_touch_lower are closed-form analytic formulas with no
    randomness — calling them multiple times returns the exact same
    number every time, so a "stability across runs" check would always
    report std=0 and trivially pass, adding compute for zero information.
    The stability gate only has meaning where there's real sampling
    randomness to be stable against.
    """
    sym     = ts.symbol
